...
func _ready():
	btn_load_to_picker.connect("pressed", load_to_picker)
	btn_update_from_picker.connect("pressed", update_from_picker)
	grid.connect("grid_item_reordered", _grid_item_reordered)
...
#			Color rect instance properties
			...
			cri.color = c
			cri.connect("tile_selected", _on_tile_selected)
			cri.connect("tile_deleted", _on_tile_deleted)
			grid.add_child(cri)
...
func set_selected(value: bool) -> void:
	selected = value
	if selected:
		var sb: StyleBoxFlat = theme.get_stylebox("panel", '').duplicate()
